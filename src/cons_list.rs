...
impl ConsList {
    pub fn new() -> ConsList {
        ConsList{ head: Box::default(), end: std::ptr::null_mut() }
    }
    ...
    pub unsafe fn append(&mut self, val: Value) {
        if self.end.is_null() {
            let tail = Box::new(Cons { car: Some(Box::new(val)), cdr: None });
            self.head = tail;
            self.end = self.head.as_mut() as *mut Cons;
        } else {
            let mut tail = Box::new(Value::Cons(Cons { car: Some(Box::new(val)), cdr: None }));
